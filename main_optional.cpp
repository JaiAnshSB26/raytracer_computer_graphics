/*
* Please note this is the optional file with a lot of experimentation I did and a lot of junk code here and there and comments I tend to leave out for myself,
* while programming, but this file was important to commit since it contains my implementation for the optional textures part which I am super happy about!
*/
...
public:
	...
	bool intersect(const Ray& ray, Vector& P, double &t, Vector& N, Vector& hit_albedo) const {
		...
		while (!nodes_to_visit.empty()) {
			...
			if (curNode->child_left) {
				...
				for (int i = curNode->starting_triangle; i < curNode->ending_triangle; i++) {
					Vector P_cur, N_cur, albedo_cur;
					double t_cur;
					if (TriangleIntersect(ray, P_cur, t_cur, N_cur, i, albedo_cur)) {
						if (t_cur < best_inter_distance) {
							best_inter_distance = t_cur;
							P = P_cur;
							N = N_cur;
							t = t_cur;
							hit_albedo = albedo_cur;
...
public:
	...
	bool intersect(const Ray& ray, Vector& P, double& t, Vector& N, int &object_id, Vector& hit_albedo) const  {
		...
		for (int i = 0; i < objects.size(); i++) {
			Vector P_cur, N_cur, albedo_cur; //self note- debug this might break during compilation, check this block again.
			double t_cur;
			if (objects[i]->intersect(ray, P_cur, t_cur, N_cur, albedo_cur)) {
...
#pragma omp parallel for schedule(dynamic, 1)
	...
	stbi_write_png("image_optional.png", W, H, 3, &image[0], 0);
